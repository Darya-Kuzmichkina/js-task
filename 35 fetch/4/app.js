...
//и получить от сервера информацию о вероятном поле (male/female) для этого имени.
 const btn = document.querySelector('button');
        const input = document.querySelector('input');

        btn.addEventListener('click', async () => {
            const response = await fetch(`https://api.genderize.io/?name=${input.value}`, { method: 'GET' });
